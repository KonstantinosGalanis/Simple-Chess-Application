...
namespace ChessChallenge.Application
{
    ...
    {
        public static PlayerColor selectedPlayerColor;
        ...
        public ChallengeController()
        {
            ...
            StartNewGame(PlayerType.Human, PlayerType.MyBot, difficulty);
        }

        public void StartNewGame(PlayerType whiteType, PlayerType blackType, int difficulty)
        {
            EndGame(GameResult.DrawByArbiter, log: false, autoStartNextBotMatch: false);
            gameID = rng.Next();
            if(whiteType == PlayerType.Human) {
            ...
            if (RunBotsOnSeparateThread)
            {
                ...
                // Create new task
                botTaskWaitHandle = new AutoResetEvent(false);
                Task.Factory.StartNew(() => BotThinkerThread(difficulty), TaskCreationOptions.LongRunning);
            }
            // Board Setup
            board = new Board();
        ...
        void BotThinkerThread(int difficulty)
        {
            int threadID = gameID;
            ...
            while (true)
            {
                ...
                if (threadID == gameID)
                {
                    var move = GetBotMove(difficulty);
        ...
        Move GetBotMove(int difficulty)
        {
            ...
            try
            {
                ...
                API.Move move = PlayerToMove.Bot.Think(botBoard, timer, difficulty);
        ...
        private void AutoStartNextBotMatchGame(int originalGameID, System.Timers.Timer timer)
        {
            ...
            {
                StartNewGame(PlayerBlack.PlayerType, PlayerWhite.PlayerType, difficulty);
            }
            timer.Close();
        }
        ...
        public void StartNewBotMatch(PlayerType botTypeA, PlayerType botTypeB, int difficulty)
        {
            EndGame(GameResult.DrawByArbiter, log: false, autoStartNextBotMatch: false);
            botMatchGameIndex = 0;
            ...
            StartNewGame(botTypeA, botTypeB, difficulty);
        }
